...
- [x] **Short results screen.** `core/KillfeedUI.cs`'s `ResultsPanel`: shown automatically for the whole results window (no key to hold, unlike the scoreboard) — MVP, full sorted standings, every award that fired, and a live "next match in Xs" countdown. Data comes from a new persistent broadcast (`MatchServer.BroadcastMatchResults`, separate from the transient banner announcements) so it survives the whole window instead of flashing once. **Verified data reaches clients correctly**: a debug print confirmed the server and a real connected client received byte-identical parsed MVP text/awards/standings after the network round-trip. No "rematch vote" — see below.
  - **Simplification, noted honestly**: the auto-reset from Phase 4 already restarts the match at the end of the results window — there's no actual vote UI, just a countdown to an automatic restart. Matches the earlier Phase 4 note about this same simplification.
